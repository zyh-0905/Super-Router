...
| 项 | 值 |
|-----|-----|
| 入口 | Web 站点详情「质量检测」页签；任务由 checker 执行（同站点唯一活跃、全局 3 并发） |
| 阶段 | 连接性 → 协议 → 流式 → Usage → 行为（basic 只跑前三） |
| 费用 | full 最多两次小型聊天请求，可能产生少量上游费用（界面明确提示） |
| 结论 | good / attention / failed / unknown（启发式信号，不判"假模型"） |
| 实时进度 | 带认证 fetch SSE；断线退化 1s 轮询；刷新恢复进行中任务 |
| Telegram 查询 | `/quality <channel_id>` 只读最近一次结果，不触发新检测 |
| 告警联动 | 关键阶段硬失败产生 `quality_check_failed`（后续通过自动恢复） |
