...
func FormatReport(now time.Time, windowHours int, overview SystemOverview, changes alert.AlertChanges, includeRecovered, includeOngoing bool, webBaseURL string) string {
	...
	b.WriteString("统计窗口：" + FormatWindow(windowHours) + "\n\n")
	...
	b.WriteString(fmt.Sprintf("活跃告警：%d 条\n", overview.ActiveAlerts))
	b.WriteString(fmt.Sprintf("🔴 Critical：%d  🟠 Warning：%d\n", overview.CriticalAlerts, overview.WarningAlerts))

	if changes.Total() == 0 {
		b.WriteString("\n✅ " + FormatWindow(windowHours) + "没有新的告警变化。\n")
		b.WriteString(fmt.Sprintf("当前活跃告警：%d 条（Critical %d / Warning %d）\n",
...
func writeFooter(b *strings.Builder, webBaseURL string) {
	b.WriteString("\n查询：/alerts · /relay · /balance · /health · /ratio")
	if webBaseURL != "" {
