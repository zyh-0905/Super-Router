...
- **圆圈进度展示**：每个阶段一个状态环（灰=待检测、蓝旋转弧=检测中并显示全局进度百分比、绿/橙/红=通过/关注/失败），悬停显示阶段详情气泡（状态/错误/HTTP/TTFB/token 等）；不再使用底部进度条与结果描述行
- **实时进度**：带认证 fetch SSE（原生 EventSource 无法携带 Bearer）；SSE 断开自动退化 1s 轮询（后端 SSE 租约 10 分钟，到期自动重连）；刷新页面恢复进行中任务；已完成阶段状态在检测全程保留（快照增量合并），不会退回「待检测」；`prefers-reduced-motion` 禁用循环动画
- **判定语义**：聊天请求尊重站点超时配置（上限 2 分钟）；首字节超时降级为「需要关注」而非失败（实测存在非流式挂起但流式正常的中转站）；请求未获响应时行为阶段判「无法判断」；usage/behavior 在非流式证据不可用时回退复用流式证据（evidence_source 溯源）；工具能力探测不强制 tool_choice；阶段结果 details 白名单过滤，防凭据形状字段下发
- **历史与告警**：最近 5 次检测历史；关键阶段硬失败产生 `quality_check_failed` 告警（由 Checker Evaluator 统一评估生命周期，后续通过自动恢复）
