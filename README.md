...
- **epoch 写后发布**：checker 以工作 epoch 写入整轮检测结果，轮末才发布到 epoch_counter——快照读取端永远看不到半轮混合状态，历史快照归档与确定性重放语义一致
...
- 首字节前故障切换（最多 3 次候选）；**TTFB 尝试级超时使用独立 sentinel 错误**，与客户端断开严格区分，超时自动切换下一个候选（P1-02 整改）；空 SSE 响应（首字节前 EOF）视为失败切换下一候选；客户端主动断开记 `client_canceled`，不计入上游熔断样本；非流式读取超时联动取消上游请求，阻塞读可被中断
- 四态熔断器 closed → open → half_open → degraded，指数退避冷却，冷却到期自动半开并按 `half_open_probe_count` 放行探测流量自愈；**开闸判定先写入当前请求样本**（成功请求不会触发开闸），新熔断桶预建行避免并发首写丢计数
...
- **预算记账**：Redis 原子预留（键缺失时以 DB 当日已消费金额播种对账，Redis 重启不清零）；Redis 不可用时付费探针失败关闭；余额后读取失败保留预留不退款（避免「已扣费但账目为零」）；余额差值探测按站点 advisory lock 跨进程互斥，防并发串扰
- **余额检测**（默认 10m）：见下节
...
### 🧪 API 接口质量检测
- 站点详情「质量检测」页签**一键检测**：复用已保存凭据，checker 异步执行（同站点唯一活跃任务、全局 3 并发、崩溃心跳回收、最多重试 2 次）
- **阶段**：连接性（/v1/models 认证可达性）→ 协议一致性 → 流式响应（SSE/[DONE] 完整性）→ Usage 计费 → 模型行为；basic 只跑前三阶段
- **最多两次小型聊天请求**（非流式+流式），可能产生少量上游费用，界面明确提示；模型默认站点 test_model，可临时切换
- **圆圈进度展示**：每个阶段一个状态环（灰=待检测、蓝旋转弧=检测中并显示全局进度百分比、绿/橙/红=通过/关注/失败），悬停显示阶段详情气泡；不再使用底部进度条与结果描述行；检测进行中已完成阶段的颜色状态会被保留（快照增量合并），不会退回「待检测」
- **判定语义**：结论是启发式信号（good/attention/failed/unknown），不输出"一定是假模型"等确定性结论。聊天请求**尊重站点超时配置**（上限 2 分钟）；**首字节超时降级为「需要关注」**而非失败（实测存在非流式挂起但流式完全正常的中转站）；请求未得到响应时 behavior 判「无法判断」而非「空响应」；usage/behavior 在非流式证据不可用时回退复用流式证据；工具能力探测**不强制 tool_choice**（避免模型只返回工具调用污染文本判定，且部分中转站拒绝嵌套 tool_choice 格式）
- **阶段结果 details 白名单过滤**：API 只下发已知字段（code/指标计数/证据），防止上游响应回显形状的字段（含凭据）进入浏览器与 Telegram
- 关键阶段失败产生 `quality_check_failed` 告警，后续通过自动恢复
- Web 通过**带认证 fetch SSE** 展示阶段动画与实时进度（断线自动退化轮询）；Telegram `/quality <id>` 查询最近结果（只读，不触发检测）
...
- **告警生命周期持久化**：checker 每 30s 评估并 reconcile 到 `alert_events`（新出现/持续计数/严重度升级/恢复），Web 与 Telegram 共用同一数据口径；Checker 暂停时 Web 显示最近一次持久化状态与数据时间。质量检测失败告警由 Evaluator 统一评估（不再由外部单条写入触发全量 reconcile，避免误恢复其它告警）
...
- **主动查询命令**：`/alerts` `/alert <key>` `/status` `/relay [id]` `/balance [id]` `/health [id]` `/ratio [id]`——只读数据库，不触发上游调用；多实例部署由 PostgreSQL advisory lock 保证只有一个 poller/report owner；`/balance` 按**中转站归并汇总**（相同 base_url 的站点归为一行，展示账户余额与站点数；`/balance <id>` 为中转站 ID，展示账户余额 + 成员站点名列表，不展示站点各自余额）
- **站点直达测试命令**：`/sitetest <id> [模型] [max_tokens]`——测试台的「站点测试」同款（非流式+流式各一次真实推理，返回 TTFT/延迟/余额差/实测倍率），结果异步推送；模型默认站点 test_model，max_tokens 默认 128（上限 512）；受订阅者分组授权约束，可能产生少量上游费用
- **交互体验**：私聊命令菜单（编辑框「/」菜单按订阅者能力自动维护）；查询命令显示「正在输入…」；站点列表/详情/告警带**内联键盘**（一键直达站点详情、过滤严重告警、启动站点测试）；`/sitetest` 进度消息原位更新（「进行中 ⏱ 已运行 Ns」→ 最终结果 + 「🔁 再测一次」按钮），再测一次同样原位更新
...
| 站点 | 分组筛选、站点增删改（接口协议/中转站类型/默认测试模型/余额自动登录）、健康/统计/余额/倍率详情、**质量检测（一键检测 + SSE 动画 + 历史）**、倍率检测分组、上游模型列表一键映射、**中转站视图**（相同 base_url 自动归并，卡片显示账户余额，展开查看成员站点，命名可自定义） |
| 测试台 | **分组测试**：真流式请求、站点选择自动预填该站点默认测试模型、分组限定路由、路由决策信息（渠道/策略/分组/Trace ID）、请求中等待动画；**站点测试**：直达指定站点（绕过路由引擎），默认消息 hi 可自定义，展示流式/非流式延迟与 TTFT、余额差（所花余额）、实测倍率（结果仅展示不落库） |
...
| 站点直达测试 | `POST /admin/channels/:id/site-test`（非流式+流式各一次真实推理，返回延迟/TTFT/余额差/倍率，结果仅展示不落库） |
...
| 中转站归并 | `GET /admin/relay-stations`（相同 base_url 自动归并，含账户余额与成员列表）· `PATCH /admin/relay-stations/:id`（命名，空 = 恢复自动命名） |
...
│   └── logger/           # Zap 日志
├── migrations/           # SQL 迁移（001 基础 ~ 022 分组时间索引；up/down 成对，启动时自动执行）
├── configs/              # 配置文件
